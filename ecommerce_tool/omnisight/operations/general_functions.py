...
def fetchAllorders(request):
    ...
    if market_place_id and market_place_id != "all" and market_place_id == "custom":
        if search_query:
            search_query = re.escape(search_query.strip())
            pipeline.append({
                "$match": {"order_id": {"$regex": search_query, "$options": "i"}}
            })

        pipeline.append({
            "$match": {"purchase_order_date": {"$lte": now_pacific}}
        })
    ...
    if search_query:
        search_query = re.escape(search_query.strip())
        match_query = {"purchase_order_id": {"$regex": search_query, "$options": "i"}}
        pipeline.append({"$match": match_query})
        count_pipeline.append({"$match": match_query})

    if market_place_id != "custom":
        pipeline.append({
            "$match": {"order_date": {"$lte": now_pacific}}
        })
        count_pipeline.append({
            "$match": {"order_date": {"$lte": now_pacific}}
        })
        ...
        # Convert order_date to Pacific Time
        for order in orders:
            if 'order_date' in order and isinstance(order['order_date'], datetime):
                order['order_date'] = order['order_date'].astimezone(pacific).strftime("%Y-%m-%d %H:%M:%S %Z")
