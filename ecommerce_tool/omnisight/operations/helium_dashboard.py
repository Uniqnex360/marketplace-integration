...
def get_metrics_by_date_range(request):
    ...
    # Precompute date filters
    date_filters = {
        "targeted": {"start": start_date_dt, "end": end_date_dt},
        "previous": {"start": start_date_dt - timedelta(days=1), "end": end_date_dt - timedelta(days=1)}
    }
    ...
    # Precompute common query parameters
    common_params = (marketplace_id, brand_id, product_id, manufacturer_name, fulfillment_channel, timezone_str)

    # Function to process date range in parallel
    def process_date_range(key, date_range, results, is_graph_data=False):
        gross_revenue = 0
        result = grossRevenue(date_range["start"], date_range["end"], *common_params)
        if result:
            gross_revenue = sum(ins['order_total'] for ins in result if ins.get('order_total', 0) > 0)
        
        if is_graph_data:
            results[key] = {"gross_revenue": round(gross_revenue, 2)}
        else:
            refund_ins = refundOrder(date_range["start"], date_range["end"], *common_params[:-1])
            refund = sum(len(ins['order_items']) for ins in refund_ins) if refund_ins else 0
            total_orders = len(result)
            total_units = sum(ins['items_order_quantity'] for ins in result) if result else 0
            total_cogs, net_profit, margin, tax_price, temp_other_price, vendor_funding = 0, 0, 0, 0, 0, 0

            if result:
                for ins in result:
                    for j in ins['order_items']:
                        item_result = list(OrderItems.objects.aggregate(
                            {"$match": {"_id": j}},
                            {"$lookup": {
                                "from": "product",
                        ...
                        if item_result:
                            ...
                            total_cogs += item_data['total_cogs'] if ins['marketplace_name'] == "Amazon" else item_data['w_total_cogs']
    ...
    # Process last 8 days in parallel
    results_graph = {}
    threads = []
    for key, date_range in last_8_days_filter.items():
        thread = threading.Thread(target=process_date_range, args=(key, date_range, results_graph, True))
    ...
    graph_data = OrderedDict((key, results_graph[key]) for key in last_8_days_filter.keys())

    # Process targeted and previous dates sequentially (less parallelization overhead for fewer tasks)
    metrics = {"graph_data": graph_data}
    for key, date_range in date_filters.items():
        results_metrics = {}
        process_date_range(key, date_range, results_metrics)
        metrics[key] = results_metrics[key]
    ...
    if item_result:
        ...
        for metric in ['gross_revenue', 'total_units', 'total_cogs', 'total_orders', 'refund', 'margin']:
            if not item_result.get(metric, True):
                del metrics['targeted'][metric]
                del metrics['previous'][metric]
