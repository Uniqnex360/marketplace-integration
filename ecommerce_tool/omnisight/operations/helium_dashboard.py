...
def get_metrics_by_date_range(request):
    ...
    # Calculate all required date ranges at once
    date_ranges = {
        "targeted": (start_date_dt, end_date_dt),
        "previous": (start_date_dt - timedelta(days=1), end_date_dt - timedelta(days=1))
    }

    # Calculate last 8 days ranges
    eight_days_ago = start_date_dt - timedelta(days=8)
    last_8_days = {
        (eight_days_ago + timedelta(days=i)).strftime("%B %d, %Y").lower(): (
            datetime.combine(eight_days_ago + timedelta(days=i), datetime.min.time()),
            datetime.combine(eight_days_ago + timedelta(days=i), datetime.max.time())
        ) for i in range(1, 9)
    }
    ...
    async def fetch_all_revenue():
        tasks = []
        for period, (start, end) in {**date_ranges, **last_8_days}.items():
            tasks.append(asyncio.create_task(
                asyncio.to_thread(
                    grossRevenue,
                    start, end, marketplace_id, brand_id, product_id,
                    manufacturer_name, fulfillment_channel, timezone_str
                )
            ))
        return await asyncio.gather(*tasks)

    # Fetch all refund data in parallel
    ...
    revenue_data, refund_data = loop.run_until_complete(asyncio.gather(
        fetch_all_revenue(),
        fetch_all_refunds()
    ))
    loop.close()
    ...
    metrics = {"graph_data": {}}
    for i, (period, _) in enumerate(last_8_days.items()):
        metrics["graph_data"][period] = {
            "gross_revenue": round(sum(order['order_total'] for order in revenue_data[i]), 2)
        }

    # Process main metrics
    for i, (period, _) in enumerate(date_ranges.items()):
        revenue_result = [r for r in revenue_data[i + len(last_8_days)]
                        if r.get('order_status') != 'Cancelled' and r.get('order_total') > 0]
        
        # Batch process order items
        all_order_items = [item for order in revenue_result for item in order['order_items']]
        
        pipeline = [
            {"$match": {"_id": {"$in": all_order_items}}},
            {"$lookup": {
                "from": "product",
        ...
        items_data = {item['_id']: item for item in OrderItems.objects.aggregate(*pipeline)}

        # Calculate metrics
        gross_revenue = sum(order['order_total'] for order in revenue_result)
        total_units = sum(order['items_order_quantity'] for order in revenue_result)
        total_orders = len(revenue_result)
        refund = sum(len(ins['order_items']) for ins in refund_data[i])

        total_cogs = 0
        temp_other_price = 0
        vendor_funding = 0

        for order in revenue_result:
            for item_id in order['order_items']:
                item = items_data.get(item_id, {})
                temp_other_price += item.get('price', 0)
                total_cogs += item.get('total_cogs' if order['marketplace_name'] == "Amazon" else 'w_total_cogs', 0)
    ...
    metrics["difference"] = {
        key: round(metrics["targeted"][key] - metrics["previous"][key], 2)
        for key in metrics["targeted"]
    }

    # Apply matrix filters
    matrix = chooseMatrix.objects.filter(name="Today Snapshot").first()
    if matrix:
        field_mapping = {
        ...
        if not matrix.select_all:
            for matrix_field, metric_field in field_mapping.items():
                if not getattr(matrix, matrix_field):
                    for period in ['targeted', 'previous']:
                        metrics[period].pop(metric_field, None)
...
def get_individual_products(match, page, page_size, start_date, end_date, 
    ...
    # OPTIMIZATION 1: Early $project to reduce data size
    pipeline.append({
        "$project": {
    ...
    # OPTIMIZATION 2: Unified $facet for count + pagination
    facet_stage = {
        "$facet": {
    ...
    # Sort by calculated fields if needed (Python sort as last resort)
    if sort_by and sort_by in calculated_fields:
        reverse_sort = sort_by_value == -1
