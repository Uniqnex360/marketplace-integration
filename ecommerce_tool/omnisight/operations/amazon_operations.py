...
def syncRecentAmazonOrders():
    ...
    # ⚡ Step 1: Preload SKU-to-Product ID map
    ...
    for _, s in df.iterrows():
        ...
        purchase_order_id = str(s.get('amazon-order-id', ''))
        ...
        if order_obj:
            DatabaseModel.update_documents(Order.objects, {"purchase_order_id": purchase_order_id}, {
                "items_order_quantity": int(s.get('quantity', 0)),
                "shipping_price": float(s.get('shipping-price', 0.0)),
                "sales_channel": str(s.get('sales-channel', '')),
                "merchant_order_id": str(s.get('merchant-order-id', '')),
                "order_status": str(s.get('order-status', ''))
            })
            continue
        ...
        item_price = safe_convert(currency, float(s.get('item-price', 0)))
        item_tax = safe_convert(currency, float(s.get('item-tax', 0)))
        shipping_price = safe_convert(currency, float(s.get('shipping-price', 0)))
        shipping_tax = safe_convert(currency, float(s.get('shipping-tax', 0)))
        promo_discount = safe_convert(currency, float(s.get('item-promotion-discount', 0)))
        promo_discount_tax = safe_convert(currency, float(s.get('item-promotion-discount-tax', 0)))
        ...
        order_item = OrderItems(
            ...
            ProductDetails=ProductDetails(
                product_id=product_id,
                Title=str(s.get('product-name', '')),
                SKU=sku,
                ASIN=str(s.get('asin', '')),
                QuantityOrdered=int(s.get('quantity', 0)),
                QuantityShipped=int(s.get('quantity', 0)),
            ),
            Pricing=Pricing(
        ...
        order = Order(
            ...
            number_of_items_shipped = int(s.get('number-of-items')) if pd.notna(s.get('number-of-items')) else 0,
            fulfillment_channel=fulfillment_channel,
            ship_service_level=str(s.get('ship-service-level', '')),
            order_date=order_date,
            order_status=str(s.get('order-status', '')),
